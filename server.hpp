...
#define DEFAULT_LOG_LEVEL Log_Level::LOG_LEVEL_DEBUG
...
public:
    ...
    TimerTask(uint64_t task_id, uint32_t trigger_time, int rotation, const TaskFunction &task_callback, const ReleaseFunction &cleanup_callback)
        : _task_id(task_id),
          _trigger_time(trigger_time),
          _rotation(rotation),
          _is_cancel(false),
    ...
    ~TimerTask()
    {
        ...
        // 无论是自然到期还是主动取消，都需要清理 map 中残留过期的 weak_ptr
    ...
    void SetReleaseFunction(const ReleaseFunction &cleanup_callback)
    {
        _cleanup_callback = cleanup_callback;
    }
...
private:
    ...
    ReleaseFunction _cleanup_callback; // 清理相关资源
};
...
private:
    ...
    void AddTimerTask(uint64_t task_id, uint32_t trigger_time, const TaskFunction &task_callback)
    {
        ...
        TimerTaskShared timer_task_shared = std::make_shared<TimerTask>(
            ...
            task_callback, std::bind(&WheelOfTime::CleanupTimerTaskRef, this, task_id));
        // 2) 写入时间轮
    ...
    void RemoveTimerTask(uint64_t task_id)
    {
        ...
        int old_slot = it->second._slot;
        // 1. 清除索引---避免后面析构 cleanup 影响当前 iterator
    ...
    // 仅清除索引
    void CleanupTimerTaskRef(uint64_t task_id)
    {
